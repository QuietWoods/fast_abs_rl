...
def fixed_bug_instructions(content):
    """
        ...
        for line in maping:
            ...
            if from_to != "":
                ...
                with open(fulltext, 'r', encoding='utf-8') as fin, open(tokenize, 'w', encoding='utf-8') as w_out:
                    ...
                    data = json.load(fin)
                    # 解决说明书的一些断句缺陷
                    instructions = fixed_bug_instructions(data['src_instructions'])
                    # 特殊字段需要格式化
...
def format_text(input_string):
    ...
    # 替换多个空格
    # content = re.sub(pattern='\s+', repl=' ', string=content)
    # 替换权利要求里面特定的句子
    content = re.sub(pattern='[根如][据]?权利[要求]?\w+?[\W]?\w+?所述', repl='所述', string=content)
    content = re.sub(pattern='其特征[在是][于]?', repl='', string=content)
    return content
